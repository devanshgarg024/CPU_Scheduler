...
namespace rr{
    ...
    void schedular::createProcessLog(std::vector<ProcessLog *> &logs, long long startTime, long long endTime, int pid)
    {
    ...
    std::vector<ProcessLog *> schedular::schedule(std::vector<Process *> processList)
    {
        QueueService queue;
        ...
        while (!queue.is_empty())
        {
            ...
            {
                executeCpuBoundProcess(process,time_quanta, queue);
            }
            else
            {
                handleIoBoundProcess(process,10,time_quanta, queue);
            }
